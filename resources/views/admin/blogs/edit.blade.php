...
<div class="mb-6">
    ...
    <select name="category_ids[]" multiple class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" style="height: 120px;">
        @foreach($categories as $category)
            <option value="{{ $category->id }}" {{ $blog->categories->contains($category->id) || in_array($category->id, old('category_ids', [])) ? 'selected' : '' }}>{{ $category->name }}</option>
        @endforeach
    </select>
...
@include('admin.blogs.partials.tags-field', ['tags' => old('tags', $blog->tags ?? []), 'fieldId' => 'edit-blog-tags'])
        ...
        @include('admin.seo.partials.advanced-fields', [
            ...
            'titleFallback' => old('title', $blog->title),
            'descriptionFallback' => old('excerpt', $blog->excerpt),
            'canonicalFallback' => route('blog.show', $blog->slug),
            'schemaType' => 'Article',
        ])
