@php
$wishlistItems = $wishlistProducts ?? collect([]);
@endphp
        ...
        <div class="relative overflow-hidden group z-[5] before:absolute before:w-full before:h-full before:top-0 before:left-0 before:bg-title before:opacity-0 before:duration-300 before:z-[5] hover:before:opacity-80">
            ...
            <div class="absolute z-10 top-[57%] left-[65%] transform -translate-y-2/4 -translate-x-2/4 flex gap-2">
                <form action="{{ route('cart.add') }}" method="POST" class="inline">
                    ...
                    <button type="submit" class="w-9 lg:w-12 h-9 p-2 lg:h-12 bg-white dark:bg-title bg-opacity-10 flex items-center justify-center transform translate-y-8 opacity-0 transition-all group-hover:duration-500 group-hover:opacity-100 group-hover:translate-y-0 relative tooltip-icon" title="Add to Cart">
                ...
                <form action="{{ route('wishlist.remove', $product->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-9 lg:w-12 h-9 p-2 lg:h-12 bg-white dark:bg-title bg-opacity-10 flex items-center justify-center translate-y-8 opacity-0 transition-all group-hover:duration-300 group-hover:opacity-100 group-hover:translate-y-0 relative tooltip-icon">                                            
...
<a href="{{ route('product-details', $product->slug) }}" class="text-underline">
                        {{ $product->name }}
                    </a>
                </h5>
                <ul class="flex items-center gap-2 mt-1">
                    ...
                    <li>
                        <svg width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M11.1622 13.6923L7.181 11.201L3.19978 13.6922C3.05515 13.7839 2.86858 13.7769 2.72931 13.6758C2.59043 13.5751 2.52673 13.4001 2.56864 13.2337L3.70764 8.67717L0.150459 5.6612C0.0189569 5.55107 -0.0324041 5.37191 0.0206119 5.2088C0.0736279 5.04526 0.220726 4.93062 0.391668 4.9187L5.03447 4.59449L6.79065 0.23853C6.91968 -0.07951 7.44233 -0.07951 7.57136 0.23853L9.32754 4.59449L13.9703 4.9187C14.1413 4.93062 14.2884 5.04526 14.3414 5.2088C14.3944 5.37191 14.3431 5.55107 14.2115 5.6612L10.6543 8.67723L11.7933 13.2337C11.8353 13.4001 11.7716 13.5752 11.6327 13.6759C11.9358 13.7791 11.7498 13.7814 11.1622 13.6923Z" fill="#EE9818"/>
                        </svg>
                    </li>
                    <li>
                    ...
                    <li class="dark:text-gray-100">({{ $product->reviews_count ?? 0 }} reviews)</li>
                </ul>
            </div>
        </div>
    </div>
...
@else
    <div class="col-span-full text-center py-12">
        ...
        <a href="{{ url('/shop') }}" class="inline-block mt-4 btn btn-solid">
