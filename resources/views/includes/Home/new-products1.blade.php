@foreach ($products as $product)
    <div class="relative overflow-hidden group">
        <a href="{{ route('product-details', ['slug' => $product->slug]) }}">
            ...
            @else
                <img class="w-full transform duration-300 group-hover:scale-110" src="{{ asset('assets/img/no-image.png') }}"
                    alt="{{ $product->name }}">
            @endif

            <!-- Product Content -->
            <div
                class="flex flex-col items-start gap-3 md:gap-4 absolute z-20 w-11/12 bottom-3 xl:bottom-5 left-[96%] transform -translate-x-1/2 p-4 xl:p-5 bg-white dark:bg-title bg-opacity-[85%] dark:bg-opacity-[85%] group-hover:-translate-y-1/2 duration-500 group-hover:opacity-0 group-hover:invisible">

                <h4 class="font-medium leading-none dark:text-white text-lg">
                    @if($product->sale_price)
                        {{ "$" . number_format($product->sale_price, 2) }}
                        <span
                            class="text-title/50 line-through pl-2 inline-block">{{ "$" . number_format($product->price, 2) }}</span>
                    @else
                ...
                <div>
                    ...
                    <ul class="flex items-center gap-2 mt-1">
                        ...
                        <li>
                            <svg class="fill-current dark:text-white" width="15" height="14" viewBox="0 0 15 14" fill="none"
                        ...
                        <li class="dark:text-gray-100">(0)</li>
                    </ul>
                </div>
            </div>
