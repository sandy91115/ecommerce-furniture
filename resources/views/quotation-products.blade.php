...
<div class="s-py-100">
    <div class="container-fluid">
        <div class="max-w-[1720px] mx-auto flex items-start justify-between gap-8 flex-col lg:flex-row border-b border-bdr-clr dark:border-bdr-clr-drk pb-8 md:pb-[50px]">
            <div class="max-w-3xl">
                <h3 class="text-3xl md:text-4xl font-semibold leading-none">Quote-Only Furniture Collection</h3>
                <p class="mt-4 text-lg text-title/70 dark:text-white/70">Yahan sirf wo products show ho rahe hain jo custom quotation ke liye available hain. Product open karke WhatsApp se inquiry bhej sakte ho.</p>
            </div>
            <div class="w-full max-w-md rounded-[24px] border border-bdr-clr dark:border-bdr-clr-drk bg-white dark:bg-dark-secondary p-6">
                ...
                <p class="mt-2 text-sm text-title/70 dark:text-white/70">Active quotation products available right now.</p>
            </div>
        </div>
        ...
        <div class="max-w-[1720px] mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-8 pt-8 md:pt-[50px]" data-aos="fade-up" data-aos-delay="200">
            @forelse($products as $product)
                <div class="group bg-white dark:bg-dark-secondary rounded-xl shadow-lg hover:shadow-xl transition-all overflow-hidden relative">
                    ...
                    <div class="p-6 relative z-20">
                        ...
                        @if($product->short_description)
                            <p class="text-sm text-title/70 dark:text-white/70 mb-5 line-clamp-3">{!! strip_tags($product->short_description) !!}</p>
                        @endif
                        <div class="flex items-center justify-between gap-3">
                            <span class="text-lg font-bold text-primary">Custom quotation</span>
                            <a href="{{ route('product-details', $product->slug) }}" class="btn btn-solid btn-sm" data-text="Open Product">
            ...
            @empty
                <div class="col-span-full text-center py-16">
                    ...
                    <p class="mt-3 text-title/70 dark:text-white/70">Abhi koi quote-only product active nahi hai. Thodi der baad dobara check karo.</p>
                </div>
            @endforelse
        </div>
