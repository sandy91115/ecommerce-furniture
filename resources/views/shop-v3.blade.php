...
<div class="s-py-50-100" data-aos="fade-up">
    <div class="container-fluid">
        <!-- Top Filter Form -->
        <form method="GET" action="{{ request()->url() }}" class="flex flex-col lg:flex-row items-start lg:items-center lg:justify-center gap-6 flex-wrap mb-12">
            <!-- Category Select -->
            ...
            <!-- Price Range -->
            <div class="flex items-start sm:items-center gap-[15px] flex-wrap sm:flex-nowrap sm:max-w-[411px] w-full flex-col sm:flex-row">
                ...
                <div class="grid grid-cols-2 gap-[15px] sm:max-w-[260px] w-full">
                    <div class="py-[10px] px-5 border border-title dark:border-white-light flex items-center gap-[5px]">
                        <span class="text-title dark:text-white font-medium leading-none min-w-[30px]">Min:</span>
                        <div class="relative flex-1">
                            <span class="text-title dark:text-white font-medium leading-none absolute left-3 top-1/2 -translate-y-1/2">$</span>
                            <input name="min_price" class="pl-10 w-full appearance-none bg-transparent text-title dark:text-white font-medium leading-none border-none outline-none" type="number" value="{{ request('min_price', '') }}" placeholder="0">
                        </div>
                    </div>
                    <div class="py-[10px] px-5 border border-title dark:border-white-light flex items-center gap-[5px]">
                        <span class="text-title dark:text-white font-medium leading-none min-w-[30px]">Max:</span>
                        <div class="relative flex-1">
                            <span class="text-title dark:text-white font-medium leading-none absolute left-3 top-1/2 -translate-y-1/2">$</span>
                            <input name="max_price" class="pl-10 w-full appearance-none bg-transparent text-title dark:text-white font-medium leading-none border-none outline-none" type="number" value="{{ request('max_price', '') }}" placeholder="1000">
                        </div>
                    </div>
                </div>
            </div>
            
            <button type="submit" class="btn btn-solid px-8 min-w-[140px]">Filter</button>
        </form>
