<!-- Footer Start -->
<div class="site-footer relative bg-overlay before:bg-title before:bg-opacity-95" style="background-image: url('{{ asset('assets/img/bg/footer.jpg') }}');">
    <div class="s-pt-100">
        <div class="container-fluid">
            ...
            <div class="max-w-[1722px] mx-auto flex items-start justify-between gap-10 s-pb-100 flex-wrap lg:flex-nowrap footer-wrapper">
                ...
                <div class="lg:max-w-[366px] sm:w-[45%] lg:w-full flex items-start justify-between gap-10 footer-inner-wrapper">
                    <div>
                        ...
                        <ul class="text-white leading-none flex flex-col gap-4">
                            @forelse($menus['footer_sitemap'] ?? [] as $menu)
                            <li class="duration-100 hover:text-primary inline-block group"><a class="text-underline-primary" href="{{ url($menu->url) }}">{{ $menu->title }}</a></li>
                            @empty
                            <li>No sitemap links</li>
                            @endforelse
                        </ul>
                    </div>
                    <div>
                        ...
                        <ul class="text-white leading-none flex flex-col gap-4">
                            @forelse($menus['footer_others'] ?? [] as $menu)
                            <li class="duration-100 hover:text-primary inline-block group"><a class="text-underline-primary" href="{{ url($menu->url) }}">{{ $menu->title }}</a></li>
                            @empty
                            <li>No others links</li>
                            @endforelse
                        </ul>
                    </div>
                </div>

                <div class="site-footer__brand lg:max-w-[522px] w-full sm:text-center -order-1 lg:order-none">
                ...
                <div class="lg:max-w-[460px] sm:w-[45%] lg:w-full flex items-start justify-between gap-10 footer-inner-wrapper">
                    <div>
                        ...
                        <ul class="text-white leading-none flex flex-col gap-4">
                            @forelse($menus['footer_shop'] ?? [] as $menu)
                            <li class="duration-100 hover:text-primary inline-block group"><a class="text-underline-primary" href="{{ url($menu->url) }}">{{ $menu->title }}</a></li>
                            @empty
                            <li>No shop links</li>
                            @endforelse
                    ...
                    <div>
                        ...
                        <ul class="text-white leading-none flex flex-col gap-4">
                            @forelse($menus['footer_service'] ?? [] as $menu)
                            <li class="duration-100 hover:text-primary inline-block group"><a class="text-underline-primary" href="{{ url($menu->url) }}">{{ $menu->title }}</a></li>
                            @empty
                            <li>No service links</li>
                            @endforelse
                        </ul>
                    </div>
                </div>
            ...
            <div class="max-w-[1722px] mx-auto border-t border-white border-opacity-10 py-5 md:py-7 text-center">
                <p class="mb-0 text-white">© 2026 CAROM STUDIOS. Develop with <i class="mdi mdi-heart text-red-700"></i> by <a href="https://shreethemes.in/" target="_blank" class="text-reset">Lets Digital Marketing</a>.</p>
            </div>
        </div>
    </div>
</div>
