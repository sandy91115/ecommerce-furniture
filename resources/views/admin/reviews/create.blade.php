...
<<<<<<< HEAD
            ...
            <div>
                ...
                <input type="email" name="reviewer_email" class="form-control" value="{{ old('reviewer_email') }}" placeholder="Optional">
...
>>>>>>> a4263c56a3ac3187932f99434605d5942427c646
                ...
                <div class="flex items-center gap-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-600">
                    @for($i = 5; $i >= 1; $i--)
                        <input type="radio" name="rating" value="{{ $i }}" id="admin-star{{ $i }}" class="sr-only" required {{ old('rating') == $i ? 'checked' : '' }}>
                        <label for="admin-star{{ $i }}" class="admin-star cursor-pointer transition-all hover:scale-110" data-rating="{{ $i }}">
        ...
        <div class="flex gap-4 mt-8">
            <button type="submit" class="btn btn-primary">
                <i class="mdi mdi-content-save mr-2"></i>Create Review
            </button>
...
document.addEventListener('DOMContentLoaded', function() {
    // Admin Star Rating JS for create form
    const adminStars = document.querySelectorAll('.admin-star');
    const ratingRadios = document.querySelectorAll('input[name="rating"]');
    let currentAdminRating = {{ old('rating', 0) }};
