...
@if($blog)
            <ul class="flex items-center gap-[10px] flex-wrap">
                <li class="text-[15px] leading-none dark:text-white">{{ $blog->published_at ? $blog->published_at->format('d M, Y') : $blog->created_at->format('d M, Y') }}</li>
                @if($blog->tags && count($blog->tags) > 0)
                <li><a href="{{ url('/blog-tag') }}" class="inline-block text-title font-medium text-[15px] leading-none py-[10px] px-5 rounded-md bg-primary-midum">{{ $blog->tags[0] }}</a></li>
                @endif
            </ul>
            <h1 class="text-2xl leading-snug sm:text-3xl sm:leading-snug md:text-[40px] mt-4 md:mt-6 md:leading-snug max-w-[940px] font-bold">{{ $blog->title }}</h1>
            <!-- Author -->
            <div class="mt-4 sm:mt-5 flex items-center gap-3">
                <div class="w-12 h-12 rounded-full overflow-hidden">
                    <img class="w-full h-full" src="{{ asset('assets/img/testimonial/tmnl-02.jpg') }}" alt="author">
                </div>
                <p>Jon Doue</p>
            </div>
@else
        ...
        <div class="max-w-[1360px] mx-auto flex items-start justify-between gap-10 flex-col-reverse lg:flex-row relative">
            <div class="max-w-[940px] w-full">
                <div class="pb-7 md:pb-12">
                    @if($blog)
                    <img class="w-full" src="{{ $blog->image_url }}" alt="{{ $blog->title }}" data-aos="fade-up" data-aos-delay="100">
                    @else
...
@if($prevBlog)
                            ...
                            <a href="{{ route('blog-details-v3', $prevBlog->slug) }}" class="max-w-[80px] h-auto sm:max-w-[114px] w-full flex-none block">
                                <img class="w-full h-full object-cover" src="{{ $prevBlog->image_url }}" alt="post">
                            </a>
                            <div class="flex-1">
                                <ul class="flex items-center gap-[10px] flex-wrap">
                                    <li class="text-[15px] leading-none dark:text-white">{{ $prevBlog->published_at ? $prevBlog->published_at->format('d M, Y') : $prevBlog->created_at->format('d M, Y') }}</li>
                                    <li><a href="{{ url('/blog-tag') }}" class="inline-block text-title font-medium text-[15px] leading-none py-[10px] px-5 rounded-md group-hover:bg-primary group-hover:text-white duration-300 bg-[#dbcbbd]">{{ $prevBlog->tags[0] ?? 'Blog' }}</a></li>
                                </ul>
                                <h5 class="mt-3 font-medium dark:text-white leading-[1.5] text-xl"><a href="{{ route('blog-details-v3', $prevBlog->slug) }}" class="text-underline">{{ Str::limit($prevBlog->title, 50) }}</a></h5>
                            </div>
                            </div>
@else
                            <h6 class="tracking-[0.5em] font-semibold mb-5 md:mb-6 text-lg">PREV POST</h6>
                            ...
                            <div class="flex-1">
                                <ul class="flex items-center gap-[10px] flex-wrap">
                                    <li class="text-[15px] leading-none dark:text-white">--</li>
                                    <li><a href="{{ url('/blog-tag') }}" class="inline-block text-title font-medium text-[15px] leading-none py-[10px] px-5 rounded-md group-hover:bg-primary group-hover:text-white duration-300 bg-[#dbcbbd]">Blog</a></li>
                                </ul>
                                <h5 class="mt-3 font-medium dark:text-white leading-[1.5] text-xl"><a href="{{ url('/blog') }}" class="text-underline">No previous post available</a></h5>
                            </div>
                            </div>
@endif
                    </div>
...
@if($nextBlog)
                        ...
                        <div class="group flex sm:items-center gap-[15px]">
                            <a href="{{ route('blog-details-v3', $nextBlog->slug) }}" class="max-w-[80px] h-auto sm:max-w-[114px] w-full flex-none block">
                                <img class="w-full h-full object-cover" src="{{ $nextBlog->image_url }}" alt="post">
                            </a>
                            <div class="flex-1">
                                <ul class="flex items-center gap-[10px] flex-wrap">
                                    <li class="text-[15px] leading-none dark:text-white">{{ $nextBlog->published_at ? $nextBlog->published_at->format('d M, Y') : $nextBlog->created_at->format('d M, Y') }}</li>
                                    <li><a href="{{ url('/blog-tag') }}" class="inline-block text-title font-medium text-[15px] leading-none py-[10px] px-5 rounded-md group-hover:bg-primary group-hover:text-white duration-300 bg-[#dbcbbd]">{{ $nextBlog->tags[0] ?? 'Blog' }}</a></li>
                                </ul>
                                <h5 class="mt-3 font-medium dark:text-white leading-[1.5] text-xl"><a href="{{ route('blog-details-v3', $nextBlog->slug) }}" class="text-underline">{{ Str::limit($nextBlog->title, 50) }}</a></h5>
                            </div>
                        </div>
@else
                        <h6 class="tracking-[0.5em] font-semibold mb-5 md:mb-6 text-lg">NEXT POST</h6>
                        <div class="group flex sm:items-center gap-[15px]">
                            ...
                            <div class="flex-1">
                                ...
                                <h5 class="mt-3 font-medium dark:text-white leading-[1.5] text-xl"><a href="{{ url('/blog') }}" class="text-underline">No next post available</a></h5>
                            </div>
                        </div>
@endif
                    </div>
                ...
                <div class="s-py-50" data-aos="fade-up">
                    ...
                    <div class="p-5 sm:p-[30px] bg-[#F8F5F0] dark:bg-dark-secondary">
                        <div class="pb-5 md:pb-[30px] border-b border-bdr-clr dark:border-[#3c434a]">
                            <div class="flex flex-col sm:flex-row items-start gap-5 pb-4 md:pb-5 border-b border-bdr-clr dark:border-[#3c434a] relative">
                                <div class="w-[72px] h-[72px] rounded-full flex-none overflow-hidden">
                                    <img class="w-full object-cover" src="{{ asset('assets/img/testimonial/tmnl-01.jpg') }}" alt="comment">
                                </div>
                            ...
                            <div class="flex flex-col sm:flex-row items-start gap-5 pt-4 md:pt-5 relative ml-5 sm:ml-8">
                                <div class="w-[40px] h-[40px] rounded-full flex-none overflow-hidden">
                                    <img class="w-full object-cover" src="{{ asset('assets/img/testimonial/tmnl-02.jpg') }}" alt="comment">
                                </div>
                                <div class="max-w-[532px] w-full">
                        ...
                        <div class="flex flex-col sm:flex-row items-start gap-5 pt-5 md:pt-[30px] relative">
                            <div class="w-[72px] h-[72px] rounded-full flex-none overflow-hidden">
                                <img class="w-full object-cover" src="{{ asset('assets/img/testimonial/tmnl-03.jpg') }}" alt="comment">
                            </div>
                            <div class="max-w-[532px] w-full">
...
@if($recentBlogs && $recentBlogs->count() > 0)
                    ...
                    <div class="grid gap-[10px]">
                        ...
                        <h6 class="font-medium group text-base sm:text-lg pb-[10px] border-b border-bdr-clr dark:border-[#172430]">
                            <a class="text-underline" href="{{ route('blog-details-v3', $recent->slug) }}">{{ Str::limit($recent->title, 60) }}</a>
                        </h6>
                        @endforeach
                    </div>
@else
...
<!-- Related Posts Start -->
    ...
    <div class="container-fluid">
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 md:gap-[30px] max-w-[1720px] mx-auto" data-aos="fade-up" data-aos-delay="100">
            @include('includes.Home.blog', ['relatedBlogs' => $relatedBlogs ?? collect()])
        </div>
    </div>
</div>
<!-- Related Posts End -->
