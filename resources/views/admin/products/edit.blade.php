...
@enderror
                ...
                <div>
                    ...
                    <input type="number" name="product_rating_count" min="0" value="{{ old('product_rating_count', $product->product_rating_count) }}" placeholder="Optional" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('product_rating_count') border-red-500 @enderror">
                    @error('product_rating_count')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
<option value="rejected" {{ old('status', $product->status) == 'rejected' ? 'selected' : '' }}>Inactive</option>
            ...
            @include('admin.seo.partials.advanced-fields', [
                ...
                'titleFallback' => old('seo_title', $product->seo_title),
                'descriptionFallback' => old('seo_description', $product->seo_description),
                'canonicalFallback' => route('product-details', $product->slug),
                'schemaType' => 'Product',
            ])
            ...
            <div class="mt-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">Shipping Details (Optional)</label>
                <textarea id="shipping_details" name="shipping_details" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('shipping_details') border-red-500 @enderror">{{ old('shipping_details', $product->shipping_details) }}</textarea>
                @error('shipping_details')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>

            @include('admin.products.partials.detail-sections')
                            ...
                            <div class="relative overflow-hidden rounded-xl border bg-gray-50 shadow-sm {{ $markedForDeletion ? 'border-red-200 ring-2 ring-red-500 opacity-70' : 'border-gray-200' }}" data-existing-image-card>
                                ...
                                <div class="space-y-3 p-3 text-xs text-gray-600">
                                    ...
                                    <label class="block font-semibold text-gray-700">
                                        Alt
                                        <input type="text" name="image_meta[{{ $image->id }}][alt]" value="{{ old("image_meta.{$image->id}.alt", $image->alt) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-normal">
                                    </label>
                                    <label class="block font-semibold text-gray-700">
                                        Title
                                        <input type="text" name="image_meta[{{ $image->id }}][title]" value="{{ old("image_meta.{$image->id}.title", $image->title) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-normal">
                                    </label>
                                    <label class="block font-semibold text-gray-700">
                                        Caption
                                        <input type="text" name="image_meta[{{ $image->id }}][caption]" value="{{ old("image_meta.{$image->id}.caption", $image->caption) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-normal">
                                    </label>
                                    <label class="block font-semibold text-gray-700">
                                        Description
                                        <textarea name="image_meta[{{ $image->id }}][description]" rows="2" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-normal">{{ old("image_meta.{$image->id}.description", $image->description) }}</textarea>
                                    </label>
                                    <div class="flex items-center gap-3">
                                        <label class="block flex-1 font-semibold text-gray-700">
                                            Sort
                                            <input type="number" name="image_meta[{{ $image->id }}][sort_order]" value="{{ old("image_meta.{$image->id}.sort_order", $image->sort_order ?? $loop->index) }}" min="0" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm font-normal">
                                        </label>
                                        <label class="mt-5 inline-flex items-center gap-2 font-semibold text-gray-700">
                                            <input type="radio" name="featured_existing_image_id" value="{{ $image->id }}" {{ old('featured_existing_image_id', $product->images->firstWhere('featured', true)?->id) == $image->id ? 'checked' : '' }} class="text-blue-600">
                                            Featured
                                        </label>
                                    </div>
                                </div>
