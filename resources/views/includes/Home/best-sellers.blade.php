@php
$activeWishlistIds = array_map('intval', $wishlistIds ?? array_keys(session('wishlist', [])));
@endphp
...
<div class="group bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden">
    ...
    <div class="p-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <button class="p-3 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all duration-300 ml-2 wishlist-toggle {{ $isInWishlist ? 'added' : '' }}" data-product-id="{{ $product->id }}" title="{{ $isInWishlist ? 'Remove from wishlist' : 'Add to wishlist' }}" aria-pressed="{{ $isInWishlist ? 'true' : 'false' }}">
                <svg class="w-5 h-5 {{ $isInWishlist ? 'text-red-500' : 'text-gray-600 group-hover:text-red-500' }}" fill="currentColor" viewBox="0 0 24 21">
