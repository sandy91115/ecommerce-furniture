...
@foreach ($teams as $item)
    @php
        ...
        $item = array_merge([
            ...
            'desc' => '',
            'class' => $isImageFirst
                ? 'group sm:grid sm:grid-cols-2 bg-[#F8F5F0] dark:bg-[#1E2A35]'
                : 'group flex flex-col-reverse sm:grid sm:grid-cols-2 bg-[#F8F5F0] dark:bg-[#1E2A35]',
            'data' => $loop->index ? (string) ($loop->index * 100) : '',
            'span' => $isImageFirst ? 'false' : 'true',
            'span1' => $isImageFirst ? 'true' : 'false',
        ], $item);
    @endphp
    <div class="{{ $item['class'] }}" data-aos="fade-up" data-aos-delay="{{ $item['data'] }}">
        ...
        <div class="sm:flex items-center justify-center px-5 py-[30px]">
            <div class="sm:max-w-[294px]">
                <h4 class="font-semibold leading-none text-xl md:text-2xl duration-300 transition-all group-hover:text-primary">{{ $item['name'] }}</h4>
                ...
                <!-- <div class="flex items-center gap-5 md:gap-6 mt-[15px]">
                    <span class="text-paragraph dark:text-white" aria-hidden="true">
