    ...
    <div class="container">
        <div class="max-w-2xl mx-auto" data-aos="fade-up">
            ...
            @if (session('wishlist') && count(session('wishlist')) > 0)
                <div class="space-y-6">
                    @foreach (session('wishlist') as $id => $productId)
                        @php $product = \App\Models\Product::find($productId); @endphp
                        @if ($product)
                            <div class="flex gap-6 p-6 border border-gray-200 dark:border-gray-700 rounded-xl">
                                <a href="{{ route('product-details', $product->slug) }}" class="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden flex-shrink-0">
                                    <img src="{{ $product->images->first() ? asset('storage/' . $product->images->first()->path) : asset('assets/img/no-image.png') }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                                </a>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex gap-2">
                                            <a href="{{ route('add-to-cart', $product->id) }}" class="btn btn-primary px-6 py-2">Add to Cart</a>
                                            <form action="{{ route('remove-wishlist', $product->id) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
